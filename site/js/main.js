...
(function(){
  ...
  // Reads frames from data-frame-pattern on .hero-scroll. The video element
  // plays as an instant-on fallback while frames preload. Once enough frames
  // are loaded, the canvas fades in over the video and the video pauses.
  (function setupScrollHero(){
    const scroller = document.querySelector('.hero-scroll');
    if (!scroller) return;

    const canvas = scroller.querySelector('.hero-canvas');
    const video = scroller.querySelector('.hero-video');
    const poster = scroller.dataset.poster;

    // Always set the poster as the canvas background so something paints
    // immediately, before frames are ready.
    if (canvas && poster) {
      canvas.style.setProperty('--hero-poster', `url('${poster}')`);
    }

    // Skip frame-driven behavior when user prefers reduced motion (video keeps playing).
    ...
    // How many frames must load before we crossfade canvas in over the video.
    // On mobile, a smaller threshold means the canvas takes over sooner.
    const isMobile = window.matchMedia('(max-width: 720px)').matches;
    const READY_THRESHOLD = isMobile ? 16 : 24;
    let crossfaded = false;

    function tryCrossfade(){
      if (crossfaded || loaded < READY_THRESHOLD) return;
      crossfaded = true;
      canvas.classList.add('is-ready');
      // Pause the video and hide it after the canvas fade-in completes.
      if (video) {
        setTimeout(() => {
    ...
    function preload(){
      ...
      // then fill in the rest in order. On mobile we preload every other frame
      // first (so the rough animation is usable sooner with half the data).
      const order = [];
      // 1) first frame priority
      order.push(0);
      // 2) every Nth frame for quick rough preview
      const sparseStep = isMobile ? 6 : 10;
      for (let i = sparseStep; i < totalFrames; i += sparseStep) order.push(i);
      // 3) the rest
      for (let i = 1; i < totalFrames; i++) {
        if (i % sparseStep !== 0) order.push(i);
      }
